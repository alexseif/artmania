...
class ControllerExtensionPaymentAcceptWallets extends Controller
{
    ...
    public function index()
    {
        ...
        $this->load->model('extension/payment/accept_wallets');
        $helper = $this->model_extension_payment_accept_wallets;

        $debug          = $this->config->get('payment_accept_wallets_allow_debug');
        $order          = $this->model_checkout_order->getOrder($this->session->data['order_id']);
        $acceptToken    = $helper->requestToken($this->config->get('payment_accept_wallets_api'));
        $integration_id = $this->config->get('payment_accept_wallets_int');
        $merchant_id    = $this->config->get('payment_accept_wallets_merchant');
        ...
        $data['accept_billing_data'] = json_encode($order_data['billing_data']);
        $data['accept_billing_key']  = $acceptPayment['token'];

        return $this->load->view('extension/payment/accept_wallets', $data);
    }
    ...
    public function callback()
    {
        ...
        $hmac = $this->config->get('payment_accept_wallets_hmac');
