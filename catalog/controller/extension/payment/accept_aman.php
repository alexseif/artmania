...
class ControllerExtensionPaymentAcceptAman extends Controller
{
    ...
    public function index()
    {
        ...
        $this->load->model('extension/payment/accept_aman');
        $helper = $this->model_extension_payment_accept_aman;

        $debug          = $this->config->get('payment_accept_aman_allow_debug');
        $order          = $this->model_checkout_order->getOrder($this->session->data['order_id']);
        $acceptToken    = $helper->requestToken($this->config->get('payment_accept_aman_api'));
        $integration_id = $this->config->get('payment_accept_aman_int');
        $merchant_id    = $this->config->get('payment_accept_aman_merchant');
        ...
        $acceptKioskId = $helper->requestKioskId(
            $acceptToken['token'],
            $order_data['billing_data'],
            $acceptPayment['token']
        );

        if (!isset($acceptKioskId['pending']) ||
            isset($acceptKioskId['pending']) && $acceptKioskId['pending'] == false ||
            !isset($acceptKioskId['data']['bill_reference'])) {

            $helper->parseDeath('Failed to obtain a bill reference', $acceptKioskId);
        }

        $data['aman_id'] = $acceptKioskId['data']['bill_reference'];

        return $this->load->view('extension/payment/accept_aman', $data);
    }
    ...
    public function callback()
    {
        ...
        $hmac = $this->config->get('payment_accept_aman_hmac');
